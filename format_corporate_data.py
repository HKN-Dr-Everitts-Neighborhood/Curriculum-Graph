...
def main():
    ...
    for (i, response) in enumerate(data):
        ...
        # unidecode; otherwise we would use codecs.open with utf-8 as the
        # codec - but we really don't want the unicode on the wiki anyway,
        # and copy-pasting it properly is difficult.
        with open(output_filename, 'w') as output_file:
            ...
            for (i, (question, answer)) in enumerate(zip(questions, response)):
                ...
                # The portion of the code from here on is pretty ugly, as we're
                # special casing based on the different ranges of questions,
                # and the organization of the questions isn't clear until you
                # look at either the survey or the survey data.
                ...
                if i in range(BEGIN_COURSES, BEGIN_MISC):
                    # note split of an empty string is a list of an empty
                    # string - this is part of the reason for looking for
                    # falsey answers above.

                    # Turns out one of the classes was CS 429 - Software
                    # Engineering II, ACP. Thus we look out for this case
                    # in the regex, since otherwise the comma throws things
                    # off.
                    processed_answers = []
